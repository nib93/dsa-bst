...
class BinarySearchTree {
  ...
  find(val) {
    ...
    let found = false;
    //if node is found then return the value
    if (val === currentNode.val) return currentNode;

    //if node is not found then do pre-order traversal
    ...
    //if node not found then return undefined
  ...
  findRecursively(val) {
    ...
    //if root is empty then there is no node in the tree
